...
public class day3 extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final DatabaseReference nm= FirebaseDatabase.getInstance().getReference("events").child("quiz");
        nm.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()){
                    for (DataSnapshot npsnapshot : dataSnapshot.getChildren()){
                        artist l=npsnapshot.getValue(artist.class);
                        listData.add(l);
                    }
                    adapter= new  artistAdapter(listData,getActivity());
